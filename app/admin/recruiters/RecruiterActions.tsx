...
import { setRecruiterVerified } from '@/lib/admin/actions'
import Link from 'next/link'

export default function RecruiterActions({ id, isVerified }: { id: string; isVerified: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
      <button
        ...
        disabled={pending}
        title={isVerified ? 'Revoke verification' : 'Verify recruiter'}
        onClick={() => start(async () => { await setRecruiterVerified(id, !isVerified) })}
        style={{
