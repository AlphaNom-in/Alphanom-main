...
import { updateEmployer, setEmployerVerified, deleteEmployer } from '@/lib/admin/actions'
...
export default function UpdateEmployerForm({ employer }: { employer: Employer }) {
  ...
  function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    startSave(async () => {
      await updateEmployer(employer.id, fd)
      setSaved(true)
    })
  }
  ...
  function handleVerify() {
    startVer(async () => {
      await setEmployerVerified(employer.id, !employer.is_verified)
    })
  }
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            ...
            background: employer.is_verified ? '#FFF1F2' : '#F0FBF9',
            color:      employer.is_verified ? '#BE123C' : '#0A9E97',
          }}
        >
          {employer.is_verified
            ...
            : <><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Verify Employer</>}
        </button>
        ...
        {!confirmDelete ? (
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', padding: '6px 14px', borderRadius: '8px', background: '#FFF1F2', border: '1.5px solid #FECDD3' }}>
            <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: '#BE123C', fontWeight: 600 }}>
              Delete employer + all their jobs and candidates?
            </span>
            <button
      ...
      <form onSubmit={handleSave}>
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E4EAF1', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #F0F4FA', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.82rem', fontWeight: 700, color: '#0F1C2E', margin: 0 }}>Edit Employer Details</p>
          </div>
          <div style={{ padding: '22px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '18px' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={lbl}>Company Name</label>
              <input style={inp} name="company_name" defaultValue={employer.company_name ?? ''} required />
            </div>
            <div>
              <label style={lbl}>Email</label>
              <input style={inp} name="email" type="email" defaultValue={employer.email ?? ''} required />
            </div>
            <div>
              <label style={lbl}>Contact (Phone)</label>
              <input style={inp} name="contact_primary" defaultValue={employer.contact_primary ?? ''} />
            </div>
            <div>
              <label style={lbl}>Industry</label>
              <input style={inp} name="industry" defaultValue={employer.industry ?? ''} />
            </div>
            <div>
              <label style={lbl}>Company Address</label>
              <input style={inp} name="company_address" defaultValue={employer.company_address ?? ''} />
            </div>
          </div>
