...
export default function RecruiterForgotPasswordPage() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    try {
      await sendPasswordResetOtp(email, 'recruiter')
      setStep('otp')
  ...
  async function handleOtp(e: React.FormEvent) {
    ...
    setError(''); setLoading(true)
    try {
      await verifyPasswordResetOtp(email, otp, 'recruiter')
      setStep('password')
    } catch (err: any) { setError(err.message) }
    finally { setLoading(false) }
  }
  ...
  async function handlePassword(e: React.FormEvent) {
    ...
    setError(''); setLoading(true)
    try {
      await resetPasswordAction(email, password, 'recruiter')
      router.push('/recruiter/login?reset=1')
    } catch (err: any) { setError(err.message) }
    finally { setLoading(false) }
  }
          ...
          <div className="ap-right" style={{ padding: '2rem' }}>
            ...
            {step === 'email' && (
              <>
                ...
                <form onSubmit={handleEmail} style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                  ...
                  <BackToLogin href="/recruiter/login" />
                </form>
              </>
            )}
            ...
            {step === 'password' && (
              <>
                ...
                <p style={{ fontFamily:'var(--font-ui)', fontSize:'0.82rem', color:'#5A7A9F', margin:'0 0 1.5rem', lineHeight:1.6 }}>
                  Choose a strong password for your recruiter account.
                </p>
  ...
  return (
    <header style={{ background:'#fff', borderBottom:'1px solid #EEF3F8', position:'sticky', top:0, zIndex:100 }}>
      ...
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'0 2rem', height:'60px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <Link href="/recruiter/login" style={{ fontFamily:'var(--font-ui)', fontSize:'0.82rem', fontWeight:700, color:'#0FB9B1', textDecoration:'none', border:'1.5px solid #0FB9B1', borderRadius:'8px', padding:'6px 14px' }}>
          Back to Sign In
        </Link>
      </div>
    </header>
...
function SubmitBtn({ loading, label }: { loading: boolean; label: string }) {
  return (
    <button type="submit" disabled={loading} style={{ width:'100%', padding:'0.825rem', background: loading ? '#96AFCA' : '#0FB9B1', color:'#fff', border:'none', borderRadius:'10px', fontFamily:'var(--font-ui)', fontSize:'0.82rem', fontWeight:700, letterSpacing:'0.04em', cursor: loading ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'7px', transition:'background 0.18s', marginTop:'0.25rem' }}>
...
function BackToLogin({ href }: { href: string }) {
  return (
    <p style={{ fontFamily:'var(--font-ui)', fontSize:'0.8rem', color:'#5A7A9F', textAlign:'center', margin:0 }}>
      Remember it?{' '}
      <Link href={href} style={{ color:'#0FB9B1', fontWeight:700, textDecoration:'none', borderBottom:'1.5px solid rgba(15,185,177,0.3)' }}>Sign in</Link>
    </p>
  )
}
